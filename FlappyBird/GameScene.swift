...
//    リスタートできるようにする
    ...
    func setupItem() {
        // 壁の画像を読み込む
        let itemTexture = SKTexture(imageNamed: "bell")
        ...
        let createWallAnimation = SKAction.run({
            // 壁関連のノードを乗せるノードを作成
            let wall = SKNode()
            wall.position = CGPoint(x: self.frame.size.width + itemTexture.size().width / 2, y: 0)
            wall.zPosition = -50 // 雲より手前、地面より奥
            ...
            upper.position = CGPoint(x: 0, y: under_wall_y + itemTexture.size().height + slit_length)
